...
| L-02 | FPGA implementation | `NOT M1 EVIDENCE` | None in M1 evidence; FPGA is an execution resource, not a separate trust authority | No completed FORISEC FPGA implementation, interface, or hardware test vectors | WP4 (Logiicdev) T4.1 (FPGA acceleration + side-channel), T4.3 (HW/SW co-design), T4.4 (benchmarking/certification evidence); external acceptance may occur in an independent test environment. WP4 develops and independently accepts the FPGA configuration. Inclusion of the accepted FPGA configuration in the SOLARIX pilot remains conditional and is decided at MS9 -- Embedded Configurations Qualified and FPGA Inclusion Decision Recorded (M29) -- based on D3.4 acceptance, technical compatibility and risk. D4.5 is accepted where the MS9 decision includes the FPGA path. | No blocker for bounded TRL 5 |
...
- FPGA acceptance in an independent external test environment must not be described as SOLARIX deployment. Inclusion in the SOLARIX pilot is a separate conditional decision recorded at MS9 after compatibility and risk assessment.
...
| L-02 | WP4 (Logiicdev) T4.1, T4.3, T4.4; pilot-inclusion decision gate MS9 (M29) | Hardware module, benchmark and certification evidence reports (D4.1, D4.3, D4.4); D3.4 acceptance and recorded FPGA inclusion/exclusion decision at MS9; D4.5 applies where the FPGA path is included | M19 (D4.1, MS5) onward; MS9 pilot-inclusion decision at M29 | Only for later integrated maturity |
